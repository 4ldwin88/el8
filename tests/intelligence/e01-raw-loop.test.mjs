...
test('E01 raw-input lifecycle reaches Review through production Intelligence transitions', () => {
  ...
  // QA accelerates only time/evidence collection. It does not preconstruct the Focus or Plan.
  ...
  for (let guard = 0; guard < 8 && Number(state?.evidenceConfidence ?? 0) < .75; guard++) {
    ...
    const answerId = answerWithStateEvidence(step.question);
    assert.ok(answerId);
  ...
  const prioritization = prioritizeCandidates({memberStateRevision,candidates:[{constructId:'ACTIVITY_LEVEL',evidenceRefs:established.evidenceRefs}]});
  assert.equal(prioritization.recommended[0].constructId, 'ACTIVITY_LEVEL');

  const confirmation = confirmFocus({prioritization,decisions:[{constructId:'ACTIVITY_LEVEL',decision:'accepted',memberRank:1}],decidedAt:'2026-09-01T12:00:00Z'});
  assert.equal(confirmation.accepted[0].constructId, 'ACTIVITY_LEVEL');

  const planningInput = focusConfirmationPlanningInput(confirmation,{evidenceRefs:established.evidenceRefs,safetyDisposition:'ordinary_flow'});
  ...
  const review=reviewPlan({plan:activePlan,evidence:{adherence:'high',outcome:'improved',burden:'low',qaSimulated:true}});
