...
function normalizeEvidence(effects = []) {
  ...
  for (const effect of evidence) {
    ...
    if (effect.observationId) { if (seenObservationIds.has(effect.observationId)) continue; seenObservationIds.add(effect.observationId); }
    normalized.push(effect);
  }
  return normalized;
}
export function aggregateEvidence(effects = []) {
  let support=0, contradiction=0, actionableSupport=0, definitiveContradiction=false;
  for (const effect of normalizeEvidence(effects)) {
    const weighted=weightEffect(effect);
    if (effect.polarity==='supports') { support+=weighted; if(effect.temporality==='current'||effect.temporality==='recurring') actionableSupport+=weighted; }
...
export function classifyConcern(effects = []) {
  const evidence=aggregateEvidence(effects); let state='UNKNOWN';
...
// Driver evaluation is deliberately a separate epistemic channel. Concern observations
// cannot establish a cause merely because they strongly establish the concern itself.
// Callers must supply driver-targeted effects (scope:'driver') produced by explicit
// discrimination/confirmation evidence. Unscoped evidence remains concern evidence.
export function evaluateDriverHypothesis(effects = []) {
  const driverEffects=effects.filter(e=>e?.type==='evidence'&&e.scope==='driver');
  const result=classifyConcern(driverEffects);
  ...
  return Object.freeze({...result,established,residualUncertainty:!established,presentationRule:established?'may-present-as-supported-driver':'must-present-as-hypothesis'});
}
export function focusEligibility(effects = []) {
  ...
  return Object.freeze({...result,eligible,reason:eligible?'supported-current-or-recurring-above-floor':'insufficient-current-evidence'});
}
