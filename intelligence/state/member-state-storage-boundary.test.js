...
test('optional missing context is preserved instead of defaulted on read',()=>{
  const state=base();
  delete state.memberContext.readiness;
  delete state.memberContext.engagementSignals;
  assert.equal(Object.hasOwn(roundTrip(state).memberContext,'readiness'),false);
  assert.equal(Object.hasOwn(roundTrip(state).memberContext,'engagementSignals'),false);
});
