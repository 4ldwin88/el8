...
import { isConstructId } from './taxonomy.js';
...
export function normalizeMemberState(input, { now = new Date().toISOString() } = {}) {
  ...
  // Preserve profile/member context without treating missing capacity/readiness as medium.
  ...
  // Preserve dimension evidence/coverage conservatively. Never infer stable from absence.
  ...
  // Canonical construct-shaped records may already exist in transitional payloads.
  ...
  // Preserve facts/evidence as provenance records without inventing construct certainty.
  ...
  // Preserve hypotheses while translating only known linked semantic IDs.
  ...
  // Convert explicit member priority decisions only; recommended priorities are not active Focus.
