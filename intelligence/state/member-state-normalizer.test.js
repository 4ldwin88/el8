...
import { createMemberState, createConstructState } from './member-state-contract.js';
...
assert.equal(migrated.state.schemaVersion,undefined);
...
assert.equal(migratedV30.migrated,true);assert.equal(migratedV30.sourceSchemaVersion,'3.0.0');assert.equal(migratedV30.state.schemaVersion,undefined);assert.equal(migratedV30.state.revision,12);assert.deepEqual(migratedV30.state.constructs,v30.constructs);assert.deepEqual(migratedV30.state.facts,v30.facts);assert.deepEqual(migratedV30.state.hypotheses,v30.hypotheses);assert.deepEqual(migratedV30.state.goals,v30.goals);assert.deepEqual(migratedV30.state.constraints,v30.constraints);assert.deepEqual(migratedV30.state.memberContext,v30.memberContext);assert.deepEqual(migratedV30.state.activePlanRef,v30.activePlanRef);assert.deepEqual(migratedV30.state.activeActionIds,v30.activeActionIds);assert.deepEqual(migratedV30.state.reviewCycles,v30.reviewCycles);assert.deepEqual(migratedV30.state.historyRefs,v30.historyRefs);assert.equal(migratedV30.state.focusDecisions.SLEEP_QUALITY.decision,'deferred');assert.deepEqual(migratedV30.state.activeFocusIds,[]);assert.ok(migratedV30.notes.some(note=>note.includes('3.0.0')));

console.log('Legacy Member State representations normalize to current unversioned governed Member State without current-shape data loss');
