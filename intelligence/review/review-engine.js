...
export function reviewPlan({plan={},evidence={},safetyContextualSignals={},safetyConfirmation=null,now=Date.now(),deepenMaxDays=DEFAULT_DEEPEN_MAX_DAYS}={}){
 ...
 if(safety.pauseOrdinaryFlow)return freeze({contractVersion:'plan-review-v2',decision:'safety_hold',reason:safety.status,valid:true,safety,actionIds:freeze(actionIds),requiresDiscovery:false,requiresPlanning:false,continueCurrent:false,pausePlan:true});
 ...
 const disengaged=decision==='disengaged';return freeze({contractVersion:'plan-review-v2',decision,reason,valid:VALID.has(decision),safety,evidenceUsed:freeze({...evidence}),actionIds:freeze(actionIds),requiresDiscovery:decision==='reassess',requiresPlanning:decision==='replace'||decision==='simplify',continueCurrent:decision==='keep'||decision==='deepen',pausePlan:disengaged,memberPrompt:disengaged?'pause_or_reengage':decision==='reassess'&&reason==='deepening_timebox_expired'?'reassess_or_pause':null,deepening:{maxDays:deepenMaxDays,expired:Boolean(deepeningExpired)}})}
