...
export function reviewPlan({plan={},evidence={},measurementContract={},safetyContextualSignals={},safetyConfirmation=null,now=Date.now(),deepenMaxDays=DEFAULT_DEEPEN_MAX_DAYS}={}){
 ...
 if(safety.pauseOrdinaryFlow)return freeze({contractVersion:'plan-review-v3',decision:'safety_hold',reason:safety.status,valid:true,safety,actionIds:freeze(actionIds),requiresDiscovery:false,requiresPlanning:false,continueCurrent:false,pausePlan:true});
 const classification=classifyReviewEvidence({evidence,measurementContract});
 ...
 const disengaged=decision==='disengaged';return freeze({contractVersion:'plan-review-v3',decision,reason,valid:VALID.has(decision),safety,classification,evidenceUsed:freeze({...evidence}),actionIds:freeze(actionIds),requiresDiscovery:decision==='reassess',requiresPlanning:decision==='replace'||decision==='simplify',continueCurrent:decision==='keep'||decision==='deepen',pausePlan:disengaged,memberPrompt:disengaged?'pause_or_reengage':decision==='reassess'&&reason==='deepening_timebox_expired'?'reassess_or_pause':null,nextEvidenceRequest:decision==='deepen'?classification.sufficiency.nextEvidenceRequest:null,deepening:{maxDays:deepenMaxDays,expired:Boolean(deepeningExpired)}})}
