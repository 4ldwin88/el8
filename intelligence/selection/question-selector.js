// EL8 adaptive question selector v0.3
...
// A safety-relevant question is NOT automatically a safety override. Override requires
// an explicit rule plus an active safety condition supplied by the caller.
...
export function scoreQuestion(question,context={}){
 ...
 const value=uncertainty*iv*action*freshness*trigger,penalty=burden*.22+redundancy*.45+friction*burden*.45,score=override?Math.max(value-penalty,.75):value-penalty;
 return{eligible:true,score,components:{uncertainty,informationValue:iv,actionability:action,freshnessNeed:freshness,triggerFit:trigger,burden,redundancy,friction,signal:signalOf(question),safetyRelevant:safetyRelevant(question),safetyOverride:override}}
}
