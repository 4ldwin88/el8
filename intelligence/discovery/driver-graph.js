...
export function projectDriverGraph({memberStateRevision=null,constructStates=[],areaPriority=[],relationshipEvidence={},unresolvedRequirements=[]}={}){
 const nodes=constructStates.filter(s=>!s.excluded).map(s=>({constructId:s.constructId,dimensionIds:s.dimensionIds??[],evidenceRefs:s.evidenceRefs??[],sourceAreaIds:s.sourceAreaIds??s.dimensionIds??[],memberImportance:s.memberImportance??s.memberImportanceRank??null,severity:s.severity??null,materiality:s.materiality??null,status:(s.evidenceRefs?.length??0)?'supported':'plausible',provenanceRefs:s.provenanceRefs??[]}));
