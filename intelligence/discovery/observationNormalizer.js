...
const specificityForRole=role=>({general:0,orientation:0,'concern-scope':1,'state-probe':2,'condition-probe':2,'impact-probe':2,'urgency-probe':2,'resilience-probe':2,'control-probe':2,discriminator:2,'driver-discriminator':3,'feasibility-probe':3,'obligation-probe':3,contradiction:2,correction:2,'handoff-validation':1,'healthy-verification':1,'safety-probe':3}[role]??1);
const constructFor=(q,e)=>e.constructId??q.constructId??null;

const ROUTE_CONSTRUCTS=Object.freeze({Financial:Object.freeze(['FINANCIAL_STRAIN']),Occupational:Object.freeze(['JOB_SECURITY','SCHEDULE_DISRUPTION']),Environmental:Object.freeze(['ENVIRONMENTAL_INTERFERENCE','HOUSING_STABILITY']),PHYSICAL_CONDITION:Object.freeze(['PHYSICAL_CONDITION']),ENERGY_FUNCTION:Object.freeze(['ENERGY_FUNCTION']),SLEEP_QUALITY:Object.freeze(['SLEEP_QUALITY']),PRESSURE_PATTERN:Object.freeze(['PRESSURE_PATTERN']),RELATIONSHIP_STRAIN:Object.freeze(['RELATIONSHIP_STRAIN']),social_support_loneliness:Object.freeze(['SUPPORT_AVAILABILITY','LONELINESS']),focus_activation:Object.freeze(['FOCUS_FUNCTION','ACTIVATION']),direction_meaning_next_step:Object.freeze(['DIRECTION_CLARITY','MEANING_PURPOSE','VALUES_CLARITY','NEXT_STEP_CLARITY'])});
...
export function answerEffects(question,answerIds){return normalizeAnswerIds(question,answerIds).map(id=>ANSWER_BY_ID[id]?.effect).filter(Boolean);}
export function safetyContextForAnswer(question,answerIds){const contextualSignals={};let requiresImmediacyClarification=false;for(const effect of answerEffects(question,answerIds)){if(effect.kind==='safety_trigger'){contextualSignals.explicitSafetyConcern=true;requiresImmediacyClarification=requiresImmediacyClarification||effect.requiresImmediacyClarification===true;}else if(effect.kind==='safety_uncertain'){contextualSignals[effect.key??'safety_uncertain']=.9;}else if(effect.kind==='safety_context'&&effect.value==='mostly_safe'){contextualSignals[effect.key??'safety_context']=.65;}}
 return Object.freeze({contextualSignals:Object.freeze(contextualSignals),requiresImmediacyClarification});}
export function constructsForAnswer(question,answerIds){const constructs=[];for(const effect of answerEffects(question,answerIds)){if(effect.kind==='routing_prior'&&effect.route)constructs.push(...(ROUTE_CONSTRUCTS[effect.route]??[]));if(effect.kind==='routing'&&effect.value){const value=String(effect.value);if(value==='body_weight_scope_candidate')constructs.push('BODY_WEIGHT_CONCERN');if(value==='activity_scope_candidate')constructs.push('ACTIVITY_LEVEL');if(value==='physical_symptom_scope_candidate')constructs.push('PHYSICAL_CONDITION');}}
