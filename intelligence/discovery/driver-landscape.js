...
export function buildDriverLandscape({seedConstructIds=[],knownConstructIds=[],deferredConstructIds=[],memberSelections={},maxExpansion=24}={}){
 const deferred=new Set(deferredConstructIds);const seeds=new Set(seedConstructIds.filter(id=>!deferred.has(id)));const known=new Set([...knownConstructIds,...seedConstructIds]);const candidates=new Map();
 const add=(id,source)=>{if(!id||seeds.has(id)||deferred.has(id))return;const prior=candidates.get(id)??{constructId:id,sourcePaths:[],relationshipIds:[],memberDisposition:null};prior.sourcePaths.push(source.path);if(source.relationshipId)prior.relationshipIds.push(source.relationshipId);prior.memberDisposition=memberSelections[id]??prior.memberDisposition;candidates.set(id,prior);};
