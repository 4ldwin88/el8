...
export function deriveConcernState(observationLog,concernId){
 const observations=observationLog.filter(o=>o.concernId===concernId||o.effects?.some(e=>e.target===concernId));const effects=observations.flatMap(o=>(o.effects??[]).map(e=>({...e,questionId:e.questionId??o.questionId})));const evidence=effects.filter(e=>e.type==='evidence'&&e.target===concernId);const excluded=evidence.some(e=>e.polarity==='contradicts'&&e.certainty==='definitive');
 const evidenceSummary=Object.freeze({supports:Object.freeze(evidence.filter(e=>e.polarity==='supports').map(e=>e.questionId).filter(Boolean)),contradicts:Object.freeze(evidence.filter(e=>e.polarity==='contradicts').map(e=>e.questionId).filter(Boolean)),neutral:Object.freeze(evidence.filter(e=>e.polarity==='neutral').map(e=>e.questionId).filter(Boolean))});
