...
export function selectNextQuestion({candidates,states,recentQuestions=[]}){
 ...
 if(evidenceRequired.length){evidenceRequired.sort((a,b)=>compareEvidenceNeed(a,b,stateById));return{type:'question',question:evidenceRequired[0],reason:'required-evidence-contract'}}
 const required=eligible.filter(q=>requiredForHandoff(q)&&unresolvedQuestion(q,stateForQuestion(q,stateById)));
 if(required.length){required.sort((a,b)=>compareEvidenceNeed(a,b,stateById));return{type:'question',question:required[0],reason:'required-handoff-evidence'}}
 const unresolved=eligible.filter(q=>unresolvedQuestion(q,stateForQuestion(q,stateById)));
 if(unresolved.length){unresolved.sort((a,b)=>compareEvidenceNeed(a,b,stateById));return{type:'question',question:unresolved[0],reason:'unresolved-clarification'}}
