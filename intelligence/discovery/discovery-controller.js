...
export function createDiscoverySession({concernIds=[],questionBank=[],labels={},outerGuardrail=null,orientationLimit=null,facts={}}={}){return{version:DISCOVERY_VERSION,observationLog:[],facts:{...facts},concernIds:[...concernIds],questionBank,labels,asked:[],questionsAsked:0,outerGuardrail:Number.isInteger(outerGuardrail)&&outerGuardrail>=0?outerGuardrail:null,orientationLimit:Number.isInteger(orientationLimit)&&orientationLimit>=0?orientationLimit:null,phase:'orient',triaged:false,incomplete:false,resolutionStates:{},driverKnown:{},recoveryAttempts:{},priorityResolutionUsed:false}}
export function activateConcerns(session,concernIds=[]){session.concernIds=[...new Set([...session.concernIds,...concernIds.filter(Boolean)])];for(const id of concernIds)if(!session.resolutionStates[id])session.resolutionStates[id]='triaged';return session}
...
function unresolved(states){return states.filter(s=>!s.excluded&&!['sufficient','resolved','deferred','nonIssue','escalated'].includes(s.resolutionState))}
...
function recoveryCandidates(session,states){const open=unresolved(states).sort((a,b)=>(b.safetyEscalationLevel??0)-(a.safetyEscalationLevel??0)||(b.memberImportanceRank??0)-(a.memberImportanceRank??0)||a.concernId.localeCompare(b.concernId));for(const state of open){if((session.recoveryAttempts[state.concernId]??0)>=1)continue;const candidates=session.questionBank.filter(q=>!['gateway','orientation'].includes(q.role)&&!session.asked.includes(q.id)&&q.concernId===state.concernId&&(!q.prerequisite||q.prerequisite(state,session.observationLog)));const eligible=eligibleQuestions(candidates,[state],session.observationLog,session.facts||{}).sort((a,b)=>(b.specificityLevel??0)-(a.specificityLevel??0));if(eligible.length){session.recoveryAttempts[state.concernId]=(session.recoveryAttempts[state.concernId]??0)+1;return eligible}}return[]}
// State-probe redundancy is established by explicit semantic facts in question eligibility or by the
// probe already having been asked. A generic evidence-confidence score must not suppress a question.
function redundantStateProbe(){return false}
...
export function nextDiscoveryStep(session){
 ...
 const stop=stoppingDecision({states,questionsAsked:session.questionsAsked,outerGuardrail:session.outerGuardrail});if(stop.stop){session.incomplete=Boolean(stop.incomplete);return{type:'finish',stop,states}}
 ...
 const openIds=openConcernIds(states);let candidates=eligibleQuestions(session.questionBank.filter(q=>!['gateway','orientation'].includes(q.role)&&!session.asked.includes(q.id)&&questionTouchesOpenConcern(q,openIds)&&!redundantStateProbe(q,states)),states,session.observationLog,session.facts||{}).map(q=>({...q,eligible:true})),recovery=false;
 if(!candidates.length){candidates=recoveryCandidates(session,states).filter(q=>!redundantStateProbe(q,states)).map(q=>({...q,eligible:true}));recovery=Boolean(candidates.length)}
 ...
 const recentQuestions=session.asked.map(id=>session.questionBank.find(q=>q.id===id)).filter(Boolean);const decision=selectNextQuestion({candidates,states,recentQuestions,facts:session.facts||{}});
 if(decision.type==='question'){session.asked=[...session.asked,decision.question.id];session.questionsAsked++;return{...decision,reason:recovery?'specificity-recovery':decision.reason,states}}
 if(decision.type==='escalate-safety')return{...decision,states};
 return{type:'finish',stop:{reason:decision.reason??'scheduler-sufficient',incomplete:false},states};
}
