...
export const DEFAULT_ORIENTATION_LIMIT=2;
export function createDiscoverySession({concernIds=[],questionBank=[],labels={},outerGuardrail=18,orientationLimit=DEFAULT_ORIENTATION_LIMIT,facts={}}={}){return{version:DISCOVERY_VERSION,observationLog:[],facts:{...facts},concernIds:[...concernIds],questionBank,labels,asked:[],questionsAsked:0,outerGuardrail,orientationLimit,phase:'orient',triaged:false,incomplete:false,resolutionStates:{},driverKnown:{},recoveryAttempts:{},priorityResolutionUsed:false}}
...
function nextOrientationQuestion(session){
 if(orientationAsked(session)>=session.orientationLimit)return null;
...
export function nextDiscoveryStep(session){
 ...
 if(session.phase==='narrow'&&session.concernIds.length===0){const q=gatewayQuestion(session);if(!q)return{type:'finish',stop:{reason:'no-gateway-question',incomplete:true},states:[]};session.asked=[...session.asked,q.id];session.questionsAsked++;return{type:'question',question:q,reason:'gateway',states:[]}}
