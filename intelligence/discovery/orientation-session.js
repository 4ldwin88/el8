...
function baselineDriverTriage(session){const dimensions=new Set(baselineNeedsNarrowing(session));if(!dimensions.size||session.baselineDriverTriaged)return null;const questions=session.questionBank.filter(q=>q.role==='baseline-discriminator'&&dimensions.has(String(q.dimension||'').toUpperCase())&&!session.asked.includes(q.id));if(!questions.length)return null;session.asked=[...session.asked,...questions.map(q=>q.id)];session.questionsAsked++;session.baselineDriverTriaged=true;return{type:'driver-triage',interaction:'compact-driver-relationship-screen',questions,dimensions:[...dimensions],reason:'orientation-driver-hypothesis-routing',presentation:{memberFacingAsOneInteraction:true,compactWrappingButtons:true,combineOptions:false,showDimensionLabels:false},states:[]}}
