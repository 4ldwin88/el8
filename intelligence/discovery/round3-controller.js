...
export function nextRound3Step(session) {
 ...
 let candidates=eligibleQuestions(session.questionBank.filter(q=>q.role!=='gateway'&&!session.asked.includes(q.id)&&!redundantStateProbe(q,states)),states,session.observationLog).map(q=>({...q,eligible:true}));let recovery=false;
