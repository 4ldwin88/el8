...
test('canonical projection carries feasibility constraints and supports',()=>{
 const log=[createObservation({questionId:'q-fit',concernId:'activity',effects:[
  {type:'feasibility',target:'activity',feasibility:{capacity:'low',scheduleFlexibility:'low'}},
  {type:'constraint',target:'activity',value:'limited_transport'},
  {type:'support',target:'activity',value:'partner_support'}
 ]})];
 const state=deriveConcernState(log,'activity');
 assert.equal(state.feasibility.values.capacity,'low'); assert.equal(state.feasibility.values.scheduleFlexibility,'low');
 assert.deepEqual(state.feasibility.constraints,['limited_transport']); assert.deepEqual(state.feasibility.supports,['partner_support']);
 assert.deepEqual(state.feasibility.evidenceRefs,['q-fit','q-fit','q-fit']);
});
