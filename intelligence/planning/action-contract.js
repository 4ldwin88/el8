...
export function createActionDefinition(input){
 ...
 return Object.freeze({...input,contractVersion:ACTION_CONTRACT_VERSION,actionId:input.actionId,name:input.name,actionScope,constructIds,dimensionIds:arr(input.dimensionIds??[],'dimensionIds'),eligibility:Object.freeze({...obj(input.eligibility??{},'eligibility'),minimumEvidenceRefs:arr(input.eligibility?.minimumEvidenceRefs??[],'minimumEvidenceRefs')}),exclusions:Object.freeze({...obj(input.exclusions??{},'exclusions'),contraindications:arr(input.exclusions?.contraindications??[],'contraindications')}),burden:Object.freeze({...obj(input.burden??{},'burden')}),measurement:Object.freeze({...obj(input.measurement??{},'measurement')}),review:Object.freeze({...obj(input.review??{},'review'),allowedDispositions:arr(input.review?.allowedDispositions??[],'allowedDispositions')}),stopReconsider:arr(input.stopReconsider??[],'stopReconsider'),evidenceSourceIds:arr(input.evidenceSourceIds??[],'evidenceSourceIds')});
}
