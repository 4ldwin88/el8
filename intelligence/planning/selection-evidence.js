...
export const SELECTION_EVIDENCE=freeze({
 ...
 P03:[selection({id:'money-current-state',evidenceKey:'financial.current_snapshot',purpose:'action_choice',prompt:'What would be most useful for your money situation right now?',options:['See the full picture of income, required expenses and debt','Work out which bills or payments need attention first','Take action on a specific bill or debt','Focus on increasing income','Something else']})],
 ...
 P05:[selection({id:'income-current-route',evidenceKey:'work.current_income_route',purpose:'action_choice',responseMode:'multi',prompt:'Because you said your income or work situation needs attention, which routes could realistically fit you right now? Select any that apply.',options:['Apply for jobs','Freelance or contract work','Build or sell something','Improve current work or hours','Training or credentials may be needed','I am not sure yet']})],
...
export function missingSelectionRequirements(evidence=[],answers={}){return selectionRequirementsForEvidence(evidence).filter(r=>answers?.[r.evidenceKey]==null||answers?.[r.evidenceKey]===''||(Array.isArray(answers?.[r.evidenceKey])&&!answers[r.evidenceKey].length));}
