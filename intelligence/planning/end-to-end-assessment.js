...
import { buildPlan, respondToItem } from './plan-engine.js';
import { interpretOutcome } from '../outcomes/outcomes.js';
...
export function discoveryToPlan(discoverySession, context={}) {
  ...
  const ranked=(trace.ranked||[]).map((x,i)=>({
    id:x.id,
    confidence:boundedConfidence(x.confidence ?? x.score),
    evidenceScore:x.score ?? x.confidence ?? null,
    rank:i+1,
...
export function runAssessment(answerer,context={}) {
  const session=Discovery.session(context.prior||{}),transcript=[];
  for(let step=0;step<(context.maxQuestions||8);step++){
    const q=Discovery.next(session); if(!q) break;
    const value=answerer(q,{step,session}); if(value==null) break;
    Discovery.answer(session,q.id,value); transcript.push({questionId:q.id,value});
  }
...
export function respondToAssessmentPlan(result,itemId,response,context={}) {
  ...
  return {...result,plan:respondToItem(result.plan,itemId,response,context)};
}

// Review consumes the canonical Outcome interpretation instead of maintaining another
// adherence/benefit decision tree in the assessment bridge.
export function reviewAssessmentPlan(result,outcome={}) {
  if(!result?.plan) return result;
  const decision=interpretOutcome(outcome);
  const plan={...result.plan,adaptation:decision.adaptation,failureAttribution:decision.attribution};
  if(decision.adaptation==='escalate') Object.assign(plan,{status:'escalate',active:[],actions:[]});
  return {...result,plan};
}

export function assessmentPlanView(result={}) {
  const plan=result.plan||{};
  return {status:plan.status||'observe',active:(plan.active||plan.actions||[]).map(item=>({id:item.id,type:item.type||(item.informationGathering?'log':'action'),title:item.title,goal:item.goal,measurement:item.measurement,reviewDays:item.reviewDays,rationale:item.rationale})),backlogCount:(plan.backlog||[]).length,canReplace:(plan.backlog||[]).length>0,reviewDays:plan.reviewDays||null,uncertainty:plan.uncertainty||null};
}
