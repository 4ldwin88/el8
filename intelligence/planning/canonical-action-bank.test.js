...
import { INTERVENTION_BANK, INTERVENTION_BY_ID } from '../../registries/interventions/interventionBank.js';
import { createActionRegistry } from './action-registry.js';

assert.equal(INTERVENTION_BANK.length,41);
assert.equal(new Set(INTERVENTION_BANK.map(x=>x.actionId)).size,41);
for(const action of INTERVENTION_BANK){
...
assert.equal(INTERVENTION_BY_ID['SOC-A04'].status,'held');assert.equal(INTERVENTION_BY_ID['INT-A04'].status,'held');assert.equal(INTERVENTION_BY_ID['XDM-A01'].name,'Reduce simultaneous plan demands');assert.equal(INTERVENTION_BY_ID['PHY-A03'].actionScope,'plan');
const registry=createActionRegistry(INTERVENTION_BANK);let r=registry.eligibleFor({focusIds:['ACTIVITY_LEVEL']});assert.ok(r.eligible.some(x=>x.actionId==='PHY-A02'));assert.ok(r.rejected.find(x=>x.actionId==='XDM-A01').reasonCodes.includes('requires_explicit_planning_condition'));r=registry.eligibleFor({focusIds:['ACTIVITY_LEVEL'],approvedPlanActionIds:['XDM-A01']});assert.ok(r.eligible.some(x=>x.actionId==='XDM-A01'));assert.equal(registry.forConstruct('RELATIONSHIP_STRAIN').length,0);assert.equal(registry.forConstruct('RELATIONSHIP_STRAIN',{includeHeld:true})[0].actionId,'SOC-A04');
console.log('All 41 Drive Actions are governed by the canonical intervention registry');
