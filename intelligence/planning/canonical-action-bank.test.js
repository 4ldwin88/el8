...
for(const action of CANONICAL_ACTION_BANK){
 assert.ok(action.trackingRequirement,`${action.actionId} tracking required`);
 assert.ok(action.iconKey,`${action.actionId} icon required`);
 assert.ok(action.permittedRationaleClaim,`${action.actionId} claim ceiling required`);
...
assert.equal(CANONICAL_ACTION_BY_ID['SOC-004'].status,'held');
assert.equal(CANONICAL_ACTION_BY_ID['INT-004'].status,'held');
assert.equal(CANONICAL_ACTION_BY_ID['XDM-001'].name,'Reduce simultaneous plan demands');
assert.equal(CANONICAL_ACTION_BY_ID['PHY-003'].actionScope,'plan'); // hydration has no justified canonical construct yet

const registry=createActionRegistry(CANONICAL_ACTION_BANK);
let r=registry.eligibleFor({focusIds:['ACTIVITY_LEVEL']});
assert.ok(r.eligible.some(x=>x.actionId==='PHY-002'));
assert.ok(r.rejected.find(x=>x.actionId==='XDM-001').reasonCodes.includes('requires_explicit_planning_condition'));
r=registry.eligibleFor({focusIds:['ACTIVITY_LEVEL'],approvedPlanActionIds:['XDM-001']});
assert.ok(r.eligible.some(x=>x.actionId==='XDM-001'));
