-- Reconciled remote migration: canonical_member_state_v3
-- Applied to Supabase 2026-08-30. Historical rows were verified before cutover.
...
set schema_version = coalesce(schema_version, state->>'schemaVersion')
where schema_version is null;

do $$
begin
  if exists (
    select 1 from public.el8_member_state
    where schema_version is distinct from '3.0.0'
       or state->>'schemaVersion' is distinct from '3.0.0'
       or revision is distinct from nullif(state->>'revision','')::integer
  ) then
    raise exception 'el8_member_state contains rows that are not canonical Member State v3';
  end if;
end
$$;

alter table public.el8_member_state
  alter column schema_version set default '3.0.0',
  alter column schema_version set not null;

alter table public.el8_member_state
  drop constraint if exists el8_member_state_schema_version_check;
alter table public.el8_member_state
  add constraint el8_member_state_schema_version_check
  check (schema_version = '3.0.0' and state->>'schemaVersion' = schema_version);
...
begin
  ...
  if expected_revision = -1 then
    if next_revision <> 0 then raise exception 'initial Member State revision must be 0'; end if;
  ...
  if saved.user_id is null then raise exception 'Member State revision conflict'; end if;
  return saved;
end
$$;
