-- STAGED MIGRATION DRAFT — DO NOT APPLY DIRECTLY.
-- Promote this SQL through `supabase migration new` when database writes are unfrozen.
-- Canonical Member State schema: 3.0.0.
...
-- Cutover precondition: existing rows must already have been normalized to v3.
do $$
begin
  if exists (
    select 1
...
begin
  ...
  if expected_revision = -1 then
    ...
    values(uid,'3.0.0',next_revision,next_state,now())
