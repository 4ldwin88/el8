...
for(const answer of ANSWER_BANK){assert.ok(QUESTION_BY_ID[answer.parentId],`${answer.id} is an orphan runnable answer`);assert.ok(answer.text,`${answer.id} missing text`);assert.ok(answer.effect&&typeof answer.effect==='object',`${answer.id} missing governed effect semantics`);}
...
const requiredDomainAnchors=['GEN001','PHY001','EMT001','FIN001','INT001','OCC001','SOC001','ENV001','SPT001'];
for(const id of requiredDomainAnchors)assert.ok(QUESTION_BY_ID[id],`missing governed domain anchor ${id}`);
for(const id of ['SFT000','SFT001','SFT002'])assert.equal(QUESTION_BY_ID[id]?.status,'design-only',`${id} must remain design-only pending expert validation`);
