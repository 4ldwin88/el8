...
const focusOf=p=>Array.isArray(p.focus_dimensions)&&p.focus_dimensions.length?p.focus_dimensions:[p.dimension,p.supporting_dimension].filter(Boolean).map((dimension,i)=>({dimension,role:i?'linked':'lead'}));
const interventionsOf=p=>Array.isArray(p.interventions)&&p.interventions.length?p.interventions:(Array.isArray(p.actions)?p.actions:[]).map((x,i)=>({id:`legacy-${i+1}`,action:x.action,dimensions:[x.dimension].filter(Boolean),order:i+1}));

export function mountPlanReviewV2({plan,session,ctx,answers:a,elements}){
 const {summary,evidence,resultTitle,resultCopy,resultWhy,error,review,newDim,note,form,result}=elements;
 const focus=focusOf(plan), interventions=interventionsOf(plan);
 ...
 review.onclick=async()=>{
  if(!a.signal||!a.adherence||!a.burden||!a.usefulness||!a.shift||(a.shift==='yes'&&!newDim.value)){error.textContent='Please answer each review question.';error.classList.remove('hidden');return;}
  const[rec,title,why]=decide(); review.disabled=true; review.textContent='Saving…';
  const payload={user_id:session.user.id,plan_id:plan.id,outcome:a.signal,adherence:a.adherence,burden:a.burden,usefulness:a.usefulness,priority_shift:a.shift,proposed_dimension:a.shift==='yes'?newDim.value:null,recommendation:rec,rationale:note.value.trim()||why,checkin_context:{recent_checkins:ctx,current_review:a,architecture:'adaptive_multidimensional_v2',focus_dimensions:focus,interventions}};
  const{data:r,error:e}=await supabase.from('el8_plan_reviews').insert(payload).select('id').single();
  if(e){error.textContent=e.message;error.classList.remove('hidden');review.disabled=false;review.textContent='Review the plan';return;}
  const{data:applied,error:ae}=await supabase.functions.invoke('apply-plan-review',{body:{review_id:r.id}});
  if(ae||!applied?.ok){error.textContent='Review saved, but the plan update could not be applied: '+(ae?.message||applied?.error||'Unknown error');error.classList.remove('hidden');review.disabled=false;review.textContent='Apply review again';return;}
  form.classList.add('hidden');result.classList.remove('hidden');resultTitle.textContent=title;resultWhy.textContent=why;
