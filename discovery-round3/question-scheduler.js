export const SCHEDULER_CONFIG=Object.freeze({OVERRIDE_MARGIN:0.45,RECENT_RELATED_WINDOW:2,RECENT_RELATED_PENALTY:0.18});
...
export function selectNextQuestion({candidates,states,recentQuestions=[],config=SCHEDULER_CONFIG}){
 ...
 const value=q=>informationValue(q)-recentRelatedPenalty(q,recentQuestions,config);const tier=q=>stateById.get(q.concernId)?.memberImportanceRank??0;const topTier=Math.max(...eligible.map(tier));const inTier=eligible.filter(q=>tier(q)===topTier).map(q=>({q,value:value(q)})).sort((a,b)=>b.value-a.value||a.q.id.localeCompare(b.q.id));const best=inTier[0];
 const challengers=eligible.filter(q=>tier(q)<topTier).map(q=>({q,value:value(q),state:stateById.get(q.concernId)})).filter(x=>unresolvedUncertainty(x.q,x.state)).sort((a,b)=>b.value-a.value||a.q.id.localeCompare(b.q.id));const challenger=challengers[0];
