...
function priorityResolution(session,states,reason){const open=unresolved(states).sort((a,b)=>(b.safetyEscalationLevel??0)-(a.safetyEscalationLevel??0)||(b.memberImportance??0)-(a.memberImportance??0)||(b.evidenceConfidence??0)-(a.evidenceConfidence??0));if(session.priorityResolutionUsed||open.length<2)return null;session.priorityResolutionUsed=true;return{type:'priority-resolution',reason,question:{id:'PRIORITY_RESOLUTION',role:'priority-resolution',text:'We’ve identified a few things that may be important right now. Which would you most like EL8 to focus on first?',maxSelections:2,concerns:open.map(s=>({id:s.concernId,label:session.labels[s.concernId]??s.concernId}))},states};}
function recoveryCandidates(session,states){const open=unresolved(states).sort((a,b)=>(b.safetyEscalationLevel??0)-(a.safetyEscalationLevel??0)||(b.memberImportance??0)-(a.memberImportance??0)||(b.evidenceConfidence??0)-(a.evidenceConfidence??0));for(const state of open){if((session.recoveryAttempts[state.concernId]??0)>=1)continue;const candidates=session.questionBank.filter(q=>q.role!=='gateway'&&!session.asked.includes(q.id)&&q.concernId===state.concernId&&(!q.prerequisite||q.prerequisite(state,session.observationLog))).sort((a,b)=>(b.specificityLevel??0)-(a.specificityLevel??0));if(candidates.length){session.recoveryAttempts[state.concernId]=(session.recoveryAttempts[state.concernId]??0)+1;return candidates;}}return [];}
export function nextRound3Step(session) {
 ...
 const states=deriveStates(session);const stop=stoppingDecision({states,questionsAsked:session.questionsAsked,outerGuardrail:session.outerGuardrail});if(stop.stop){if(stop.incomplete){const pr=priorityResolution(session,states,'guardrail-priority-resolution');if(pr)return pr;}session.incomplete=Boolean(stop.incomplete);return{type:'finish',stop,states};}
...
export function recordPriorityResolution(session,concernIds=[]){const chosen=[...new Set(concernIds)].filter(id=>session.concernIds.includes(id)).slice(0,2);const now=Date.now();for(const concernId of chosen)appendObservation(session,{id:`priority:${concernId}:${now}`,questionId:'PRIORITY_RESOLUTION',concernId,answerValue:true,specificityLevel:0,timestamp:now,effects:Object.freeze([{type:'member-priority',target:concernId,value:'focus-first',sourceType:'direct',temporality:'current'}])});session.priorityChoices=chosen;return session;}
