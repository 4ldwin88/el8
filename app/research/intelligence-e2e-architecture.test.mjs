...
assert.match(index,/location\.href='discovery\.html'/,'internal Intelligence QA must enter adaptive Discovery directly');
assert.doesNotMatch(index,/discovery-snapshot\.html/,'internal Intelligence QA must not restore the opening matrix wrapper');
assert.equal(exists('intelligence-test/discovery-snapshot.html'),false,'obsolete opening Discovery matrix must remain retired');
...
assert.match(discovery,/createDiscoverySession/,'internal Intelligence QA must support a fresh canonical Discovery session without a snapshot handoff');
assert.doesNotMatch(discovery,/Discovery opening snapshot required|location\.replace\('discovery-snapshot\.html'\)/,'adaptive Discovery must not require the retired opening matrix');
