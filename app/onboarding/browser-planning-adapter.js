...
function rows(output={}){return output.ranked||output.selected||output.priorityCandidates||output.candidates||[]}
...
export function canonicalPlanningInputFromBrowser({discoveryOutput,confirmedPriorities,memberStateRevision=1}){
 ...
 return{memberStateRevision:Number.isInteger(memberStateRevision)?memberStateRevision:1,confirmedPriorityIds:problems.map(x=>x.priorityId),memberChoice:{mode:'EXPLICIT_ACCEPTANCE'},problems,priorityOrder:problems.map((x,i)=>({priorityId:x.priorityId,rank:i+1})),priorityOrderResolved:true,constraints:{profile:[],capacity,manageability:feasibility.overall_load||null,throttle:{active:false},safety:{disposition:'ORDINARY_FLOW'}}};
}
