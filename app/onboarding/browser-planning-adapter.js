...
export function canonicalBrowserPlanView(plan={}){const actions=plan.proposedActions||plan.actions||[];return{status:plan.status||'no_plan',reason:plan.reason||null,focusIds:[...(plan.focusIds||[])],focusResolutions:clone(plan.focusResolutions||[]),deepeningRequirements:clone(plan.deepeningRequirements||[]),actions:clone(actions),backlog:clone(plan.backlog||[]),uncoveredFocusIds:[...(plan.uncoveredFocusIds||[])],burden:clone(plan.burden||{}),trackingRequirements:actions.map(a=>a.trackingRequirement).filter(Boolean),additionalAssessmentRequirements:actions.map(a=>a.additionalAssessmentRequirement).filter(Boolean),activationStatus:plan.status==='proposed'&&actions.length?'awaiting_member_acceptance':plan.status==='deepen'?'needs_plan_specific_assessment':null,decisionTrace:clone(plan.decisionTrace||{})}}
