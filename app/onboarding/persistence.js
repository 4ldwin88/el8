...
const dimensionFor=concernId=>CONCERN_DIMENSION[concernId]||'Physical';
...
export async function persistInitialPlan({userId,plan,discoveryOutput}={}){
  ...
  const{data:baseline,error:baselineError}=await supabase.from('el8_assessment_sessions').select('submitted_at').eq('user_id',userId).eq('module_type','universal_baseline').eq('status','completed').order('submitted_at',{ascending:false}).limit(1).maybeSingle();
  ...
  const focus=[...new Map(interventions.map((a,i)=>[a.dimensions[0],{dimension:a.dimensions[0],role:i===0?'lead':'linked',source:'discovery-round-3'}])).values()];
  const primary=interventions[0],supporting=interventions[1]||null,now=new Date().toISOString();
  const row={user_id:userId,baseline_completed_at:baseline?.submitted_at||now,dimension:primary.dimensions[0],supporting_dimension:supporting?.dimensions?.[0]||null,source:'discovery-round-3',primary_action:primary.action,supporting_action:supporting?.action||'',measure:primary.measurement||'Plan review evidence',review_days:plan.reviewDays,rationale:plan.explanation||null,status:'active',actions:interventions.map(a=>({action:a.action,dimension:a.dimensions[0]})),generated_from:{discovery_version:'round-3',discovery_trace:discoveryOutput?.trace||null,accepted_at:plan.acceptedAt},focus_dimensions:focus,interventions,capacity:{max_actions:2,review_days:plan.reviewDays},plan_objective:'Start with the smallest useful plan and review evidence before adapting.',is_test:false,version:1,updated_at:now};
