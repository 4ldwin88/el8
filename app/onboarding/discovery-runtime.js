...
export function submitDiscoveryRelationship(session,relationship=null){return relationship?Discovery.relate(session,relationship):Discovery.skipRelationships(session)}
...
// Discovery exposes only evidence-supported candidates. LIMITED/UNKNOWN signals remain in Member State for later evidence gathering but are not presented as Focus options.
export function discoveryPriorityCandidates(output={}){const states=output.trace?.states||[];return states.filter(s=>{if(['deferred','nonIssue','escalated'].includes(s.resolutionState)||s.excluded)return false;const confidence=qualitativeConfidence(s);return CONFIDENCE_ORDER[confidence]>=CONFIDENCE_ORDER.MODERATE}).map(s=>{const importance=normalizedImportance(s),confidence=qualitativeConfidence(s),constructId=s.constructId;return Object.freeze({constructId,label:s.label||constructId,qualitativeConfidence:confidence,evidenceSupport:evidenceSupport(confidence),memberImportance:importance,resolutionState:s.resolutionState,memberEmphasized:importance!==null&&importance>=2,inferred:importance===null&&confidence!=='UNKNOWN',evidenceRefs:s.evidenceRefs||[],relationships:s.relationships||[],feasibility:s.feasibility||null})}).sort((a,b)=>Number(b.memberImportance??-1)-Number(a.memberImportance??-1)||CONFIDENCE_ORDER[b.qualitativeConfidence]-CONFIDENCE_ORDER[a.qualitativeConfidence]||a.constructId.localeCompare(b.constructId))}
