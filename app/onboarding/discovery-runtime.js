...
// Temporary concern-keyed projection. It does not rank, recommend or select Focus.
export function discoveryPriorityCandidates(output={}){const states=output.trace?.states||[];return states.filter(s=>{if(['deferred','nonIssue','escalated'].includes(s.resolutionState)||s.excluded)return false;const importance=normalizedImportance(s),evidence=evidenceConfidence(s);return (importance!==null&&importance>0)||evidence>0}).map(s=>{const importance=normalizedImportance(s),evidence=evidenceConfidence(s);return Object.freeze({concernId:s.concernId,label:s.label||s.concernId,evidenceConfidence:evidence,evidenceSupport:evidenceSupport(evidence),memberImportance:importance,resolutionState:s.resolutionState,memberEmphasized:importance!==null&&importance>=2,inferred:importance===null&&evidence>0,evidenceRefs:s.evidenceRefs||[],feasibility:s.feasibility||null})}).sort((a,b)=>Number(b.memberImportance??-1)-Number(a.memberImportance??-1)||Number(b.evidenceConfidence)-Number(a.evidenceConfidence)||a.concernId.localeCompare(b.concernId))}
