...
function openingGateway(session){if(session.phase!=='orient'||!session.asked?.includes('O5')||session.asked?.includes('G1'))return null;if(['maintain','nothing'].includes(session.facts?.discoveryIntent))return null;const q=session.questionBank?.find(x=>x.id==='G1');if(!q)return null;session.phase='narrow';session.asked=[...new Set([...session.asked,'G1'])];session.questionsAsked++;return{type:'question',question:q,reason:'opening-gateway-after-orientation',states:[]}}
