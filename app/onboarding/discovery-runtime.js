...
function openingQuestion(question){return question?.id==='O5'?{...question,responseMode:'multi'}:question}
function openingGateway(session){if(session.phase!=='orient'||!session.asked?.includes('O5')||session.asked?.includes('G1'))return null;if(['maintain','nothing'].includes(session.facts?.discoveryIntent))return null;const q=session.questionBank?.find(x=>x.id==='G1'&&x.gatewayEligible===true);if(!q)return null;session.phase='narrow';session.asked=[...new Set([...session.asked,'G1'])];session.questionsAsked++;return{type:'question',question:q,reason:'opening-gateway-after-orientation',states:[]}}
export function nextDiscoveryStep(session){const gateway=openingGateway(session);if(gateway)return gateway;const step=Discovery.next(session);if(step.type==='question'&&step.question?.id==='W5'&&(session.baselineTopics?.work||[]).includes('finding_work')){session.asked=[...new Set([...session.asked,'W5'])];return Discovery.next(session)}if(step.type==='question')return{...step,question:openingQuestion(step.question)};return step}
export function answerDiscoveryQuestion(session,question,answerIds){return Discovery.answer(session,openingQuestion(question),answerIds)}
