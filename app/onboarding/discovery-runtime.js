...
import {needsTriage,buildTriageQuestion} from '../../intelligence/discovery/triage.js';
...
function openingTriageStep(session){if(session.triaged||!session.asked?.includes('G1')||session.questionsAsked!==1||!needsTriage(session.concernIds||[]))return null;return{type:'triage',question:buildTriageQuestion(session.concernIds,session.labels),states:[]}}
export function nextDiscoveryStep(session){const earlyTriage=openingTriageStep(session);if(earlyTriage)return earlyTriage;const step=Discovery.next(session);if(step.type==='question'&&step.question?.id==='W5'&&(session.baselineTopics?.work||[]).includes('finding_work')){session.asked=[...new Set([...session.asked,'W5'])];return Discovery.next(session)}return step}
