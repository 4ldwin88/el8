...
function constraintScore(action,handoff={}){const c=handoff.signals?.constraints;if(!c||['None','Prefer not to say'].includes(c))return 0;if(c==='Cost')return action.costBand==='free'?2:-2;if(c==='Time')return Number(action.estimatedMinutes||999)<=timeBudgetMinutes(handoff)?2:-2;if(c==='Accessibility')return action.accessibility==='movement-dependent'?-2:1;if(c==='Professional guidance')return action.professionalSupport==='possible'?1:0;return 0}
