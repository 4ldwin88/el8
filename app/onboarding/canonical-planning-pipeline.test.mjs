...
test('explicit snapshot capacity is preserved as evidence',()=>{const b=buildDiscoverySnapshotHandoff({candidate_concerns:['money'],feasibility:{capacity:'low',time:'<5 min',overall_load:'Difficult'}});const s=createDiscoveryFromSnapshot(b);assert.equal(s.facts.snapshotFeasibility.capacity,'low');assert.equal(s.facts.snapshotCapacity,'low')});
