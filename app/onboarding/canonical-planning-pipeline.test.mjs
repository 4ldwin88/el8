...
test('explicit Discovery resolution remains evidence/state rather than a Discovery-owned priority ranking',()=>{const s=createDiscoveryFromSnapshot(snapshot());setResolution(s,'money','sufficient',{driverKnown:true});const states=deriveStates(s);assert.equal(states.length,1);assert.equal(states[0].concernId,'money');assert.equal(states[0].resolutionState,'sufficient');const output=completeBoundarySession(s);assert.equal('plan' in output,false);assert.equal('ranked' in output,false);assert.equal('backlog' in output,false)});
test('established Discovery evidence crosses the boundary with provenance but no intervention choice',()=>{const s=establishMoney(createDiscoveryFromSnapshot(snapshot()));const output=completeBoundarySession(s),state=output.trace.states.find(x=>x.concernId==='money');assert.ok(state);assert.equal(state.resolutionState,'sufficient');assert.ok((state.evidenceRefs||[]).length>0);for(const forbidden of ['plan','memberPlan','candidateActions','selectedActionIds','recommendedPriorities','confirmedPriorities','interventions','ranked','backlog'])assert.equal(forbidden in output,false)});
