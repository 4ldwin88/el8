...
test('Baseline seed creates canonical Discovery session with member and feasibility context',()=>{const s=createDiscoveryFromBaseline(baseline());assert.ok(s);assert.deepEqual(s.concernIds,['money_pressure']);assert.equal(s.facts.memberPriorityConcern,'money_pressure');assert.equal(s.facts.baselineDimension,'Financial');assert.equal(s.facts.baselineFeasibility.capacity,'low')});
...
test('Discovery must establish evidence before Planning can activate anything',()=>{const s=createDiscoveryFromBaseline(baseline());setResolution(s,'money_pressure','sufficient',{driverKnown:true});const h=planningHandoffFromDiscovery(s);const p=buildPlan(h,{capacity:'low',selectionEvidence:{'financial.current_snapshot':'Understand where my money is going'}});assert.equal(p.status,'observe');assert.equal(p.reason,'insufficient_evidence')});

test('established Discovery evidence can cross handoff but selection evidence still gates action choice',()=>{const s=createDiscoveryFromBaseline(baseline());appendObservation(s,{concernId:'money_pressure',signal:'present',evidenceConfidence:.9,memberImportance:'high',memberPrioritySelected:true,immediacyClass:'time-sensitive',readiness:3,evidenceRef:'discovery-money'});setResolution(s,'money_pressure','sufficient',{driverKnown:true});const h=planningHandoffFromDiscovery(s);assert.equal(h.ranked[0].id,'money_pressure');const gated=buildPlan(h,{capacity:'low'});assert.equal(gated.status,'deepen');assert.equal(gated.reason,'selection_evidence_required')});

test('full canonical path activates only after Discovery and selection evidence',()=>{const s=createDiscoveryFromBaseline(baseline());appendObservation(s,{concernId:'money_pressure',signal:'present',evidenceConfidence:.9,memberImportance:'high',memberPrioritySelected:true,immediacyClass:'time-sensitive',readiness:3,evidenceRef:'discovery-money'});setResolution(s,'money_pressure','sufficient',{driverKnown:true});const h=planningHandoffFromDiscovery(s);const p=buildPlan(h,{capacity:'low',selectionEvidence:{'financial.current_snapshot':'Understand where my money is going'}});assert.equal(p.status,'active');assert.equal(p.active.length,1);assert.equal(p.active[0].driver,'money_pressure');assert.equal(p.active[0].id,'money_snapshot')});
