...
import {createDiscoveryFromSnapshot,buildOnboardingPlan} from './canonical-planning-pipeline.js';
...
test('raw onboarding evidence reaches durable canonical activation without legacy authority',async()=>{
 ...
 const built=buildOnboardingPlan({session:discovery,memberStateRevision:0,focusDecisions:[{constructId:'ACTIVITY_LEVEL',decision:'accepted'}]});
 ...
 assert.ok(built.plan.proposedActions.some(a=>a.actionId==='PHY-002'));
 const state=projectOnboardingMemberState({memberId:'member:e2e',constructs:built.discovery.candidates,focusDecisions:[{constructId:'ACTIVITY_LEVEL',decision:'accepted'}],memberContext:{capacity:'low'}});
 const store={state:null,plan:null};
 const result=await activateCanonicalOnboarding({userId:'member:e2e',memberState:state,plan:built.plan,applyPlan:applyCanonicalBrowserPlan,loadState:async()=>store.state?{revision:store.state.revision,state:store.state}:null,persistState:async({expectedRevision,state})=>{assert.equal(expectedRevision,store.state?store.state.revision:-1);store.state=state;return{revision:state.revision,state}},persistPlan:async({plan})=>{store.plan={...plan,id:'plan:e2e'};return{id:'plan:e2e',status:'proposed'}},activatePlan:async({planId})=>{assert.equal(planId,'plan:e2e');store.plan.status='active';return{id:planId,status:'active'}},completeOnboarding:async()=>{}});
 ...
 assert.ok(result.activeActionIds.includes('PHY-002'));
