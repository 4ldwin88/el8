...
// The member-confirmed focus is the authoritative concern identity. Discovery supplies the evidence.
// Prefer an exact state, then the recommended candidate, then a traceable related state normalized to
// the confirmed parent focus. Planning remains the sole intervention selector.
...
export function onboardingPlanContext({baselineHandoff={},capacity=null,selectionEvidence={},evidence={},rejectedActionIds=[],adaptationConstraint=null,previousActionIds=[],previousMechanisms=[],previousMaxEffort=null,safety=null,priorityOverride=null,adaptationExclusions=[]}={}){const signals=baselineHandoff?.signals||{},feasibility=signals.feasibility||{},overrideConstraints=priorityOverride?.constraints||[],inferredCapacity=capacity||priorityOverride?.capacitySignal||(feasibility.overall_load==='Overwhelming'||feasibility.readiness==='Not ready'?'low':'medium');return{capacity:inferredCapacity,constraints:[...(Array.isArray(signals.constraints)?signals.constraints:signals.constraints?[signals.constraints]:[]),...overrideConstraints],selectionEvidence,evidence,rejectedActionIds,adaptationConstraint,previousActionIds,previousMechanisms,previousMaxEffort,safety,priorityOverride,adaptationExclusions,consent:true}}
export function buildOnboardingAdaptivePlan({discoveryOutput={},recommendedPriorities=[],confirmedPriorities=[],priorityOverrideFrictions=[],baselineHandoff={},capacity=null,selectionEvidence={},evidence={},rejectedActionIds=[],adaptationConstraint=null,previousActionIds=[],previousMechanisms=[],previousMaxEffort=null,adaptationExclusions=[],safetyContextualSignals={},safetyConfirmation=null}={}){const safety=safetyGate({stage:'planning',contextualSignals:safetyContextualSignals,confirmation:safetyConfirmation,evidenceRefs:discoveryOutput.trace?.observations?.map(x=>x.id).filter(Boolean)||[],concernRefs:confirmedPriorities});if(safety.pauseOrdinaryFlow)return{status:'safety_hold',reason:safety.status,safety,active:[],actions:[],backlog:[],reviewDays:null};const priorityOverride=capturePriorityOverride({recommendedPriorities,confirmedPriorities,frictions:priorityOverrideFrictions});if(priorityOverride.overridden&&!priorityOverride.frictions.length)return{status:'deepen',reason:'priority_override_context_required',active:[],actions:[],backlog:[],reviewDays:null,priorityOverride};const selectedStates=planningEvidenceFromDiscovery(discoveryOutput,confirmedPriorities);const ranked=buildPlanningHandoff({selected:selectedStates,backlog:[]}).ranked;const plan=buildPlan({ranked},onboardingPlanContext({baselineHandoff,capacity,selectionEvidence,evidence,rejectedActionIds,adaptationConstraint,previousActionIds,previousMechanisms,previousMaxEffort,safety,priorityOverride,adaptationExclusions}));return{...plan,safety,priorityOverride}}
