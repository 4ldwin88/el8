...
import {validateMemberStateShape} from '../../intelligence/state/member-state-contract.js';
...
export function applyCanonicalBrowserPlan(state,plan,{at=new Date().toISOString(),planId=null}={}){
 try{validateMemberStateShape(state)}catch{throw new Error('canonical Member State is required')}
 if(!Number.isInteger(state.revision))throw new Error('canonical Member State is required');
